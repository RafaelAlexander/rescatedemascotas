...
public class RepoPublicacionesAprobadasTest extends AbstractPersistenceTest implements WithGlobalEntityManager {
  ...
  @Test
  public void agregamosPublicacionYLosDatosTienenCoherencia() throws SQLException, IOException {
    DatosPersonales datosPersonales = Fixture.datosPersonales();
    RepoPublicacionesAprobadas.getInstance().agregarManager(entityManager());
    entityManager().persist(datosPersonales);
    PublicacionAprobada publicacionAprobada = new PublicacionAprobada(this.rescatistaAleatorio(datosPersonales), this.nuevoAnimalEncontrado());
    RepoPublicacionesAprobadas.getInstance().agregarPublicacion(publicacionAprobada);
    assertEquals(RepoPublicacionesAprobadas.getInstance().darPublicaciones().get(0).getAnimalPerdido().getDescripcionEstado(),"Normal");
  }
  ...
  private ParticipanteRescate rescatistaAleatorio(DatosPersonales datosPersonales){
    return new ParticipanteRescate(datosPersonales);
  }
}
